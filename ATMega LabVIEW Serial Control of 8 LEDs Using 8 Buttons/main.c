...
void main(void)
{
	// Declaration and Initialization (executed once)
		...
		// Module Initialization
			// UART Initialization
				...
				// UBRRL
					UBRRL = 51; // select 9600 baudrate
					_delay_ms(100);
				// Initialization of DDIO (LEDs)
					DDRA = 0x00; // Input
					PORTA = 0xFF; // Input pull up port A
	...
	while(1)
	{
		...
		USART_vidSendData(~PINA);
		//_delay_ms(100);
		//PORTC = USART_u8ReceiveData();
		if(ReceivCompFlag == 1) // wait till the data are received from the transmitter
		{
			data = UDR;
			PORTB = (u8) data;
			_delay_ms(10);
...
u8 USART_u8ReceiveData(void)
{
	...
	data = UDR; // get the data from the data register UDR
	//SET_BIT(UCSRA, RXC); // reset the flag (not needed)
